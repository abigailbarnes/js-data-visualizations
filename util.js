...
/* svgAppend appends another d3-generated SVG to the document's "allvis" div,
...
export const svgAppend = function (label, layout, Height) {
  ...
  // add the final g in which the student-generated visualization goes
  return inner.append('g')
};
...
// takes an array of bytes and runs the equivalent of command-line cksum on it
function cksum(buf) {
  let crc = crcTable[0];
  function ckbyte(crc, v) {
    const idx = ((crc >>> 24) ^ v);
...
/* scksum processes the text contents of a URL to strip out the student code blocks, and
   processes whatever is left with the equivalent of command-line "cksum" */
export const scksum = (url) => {
  return fetch(url)
    ...
    .then((text) => {
      ...
      return cksum(encoder.encode(lines.join('\n')));
    });
}
